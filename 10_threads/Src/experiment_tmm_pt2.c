...
int main(int argc, char *argv[])
{
     ...
     */
    struct timespec startrealtime, startcputime;
    ...
    MatMulThreadStats* arrayofstats = (MatMulThreadStats*)malloc(MAX(nThreads,1)*sizeof(MatMulThreadStats));
     ...
     */
    mat_mul(c, n, m, a, p, b, nThreads, arrayofstats);
    ...
    struct timespec stoprealtime, stopcputime;
    clock_gettime(CLOCK_REALTIME, &stoprealtime);
    ...
    else
    {
        ...
        {
            printf("thread %d cpuTime: %lf\n", i, arrayofstats[i].cpuTime);
            cputimesum += arrayofstats[i].cpuTime;
            printf("thread %d row mults: %d\n", i, arrayofstats[i].nRowsDone);
        }
        printf("\nthread cpu time sum: %lf\n", cputimesum);
    }
    ...
    double totalcputime = tspecDiff(startcputime, stopcputime);
    double totalrealtime = tspecDiff(startrealtime, stoprealtime);
