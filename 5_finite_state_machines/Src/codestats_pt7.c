...
void codeStats_accumulate(struct CodeStats *codeStats, char *fileName)
{
    ...
    enum {
        ...
        INCPREPROCESSORDIRECTIVE,
        FOUNDESCAPEINCPREPROCESSORDIRECTIVE,
    } state = START;
    ...
    while ((ch = getc(f)) != EOF) {
        switch (state) {
            ...
            case START: // start state
                ...
                {
                    codeStats->cPreprocessorDirectiveCount++;
                    state = INCPREPROCESSORDIRECTIVE;
                    continue;
                }
            ...
            case INCPREPROCESSORDIRECTIVE:
                ...
                if(ch == '\\')
                {
                    state = FOUNDESCAPEINCPREPROCESSORDIRECTIVE;
                    continue;
                }
                break; // break case after INCPREPROCESSIRDIRECTIVE

            case FOUNDESCAPEINCPREPROCESSORDIRECTIVE:
                if(ch == '\n')
                {
                    codeStats->lineCount++;
                    state = INCPREPROCESSORDIRECTIVE;
